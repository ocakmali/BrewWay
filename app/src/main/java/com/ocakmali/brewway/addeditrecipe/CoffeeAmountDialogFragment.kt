...
class CoffeeAmountDialogFragment : DialogFragment(), NumberPickerDialog.OnNumberSetListener {

    private val addEditRecipeViewModel by sharedViewModel<AddEditRecipeViewModel>()

    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        val value = addEditRecipeViewModel.recipe.value?.equipment?.coffeeAmount ?: 15
        return NumberPickerDialog(context = requireActivity(),
                numberSetListener = this,
                title = R.string.set_coffee_amount,
                minValue = 0,
                value = value,
                maxValue = 999)
    }
    ...
    override fun onNumberSet(view: NumberPicker, value: Int) {
        addEditRecipeViewModel.updateCoffeeAmount(value)
    }
}